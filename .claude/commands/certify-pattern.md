...
Follow docs/workflows/theme-pattern-certification.md exactly; do not skip steps. Run `npm run validate` and the read-only staging checks (`npm run rest:check`, `npm run rest:certify`) as the workflow requires. Any live REST write requires a dry-run (`npm run rest:dry-run`) plus explicit approval first.
