...
2. Plan only this one pattern. State the chosen section spacing token (section-none/small/medium/large) and where the single editable H1 lives if this pattern owns it.
...
5. Report what changed, files changed, checks run, known issues, and the next recommended step. Do not run live REST writes or deploy.
