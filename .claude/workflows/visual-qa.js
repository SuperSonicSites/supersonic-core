...
2. Find the latest QA report: the highest-versioned file matching docs/reports/*qa*.md. Read its open-items sections — canonical headings are "Prioritized Fix List" and "Remaining Risks / Verify Visually"; if those exact headings are absent (older reports vary), fall back to the report's severity-ranked findings list. List those item titles in knownOpenFindings. Set latestReport to its path (or "").
...
Write a markdown report to docs/reports/${scope.version}${reportSuffix}.md.${dryRunOnly ? ' This is a STATIC/SOURCE-ONLY pre-check (no pages were created and no screenshots were captured) — title it clearly as a source-only pre-check, NOT a visual QA pass, and do NOT overwrite the canonical release report.' : ''} Use these canonical section headings in this order (the next run's discovery parses them): "Scope Reviewed", "Active Versions On Staging", "Method" (this workflow + model tiers + the create/capture/trash page lifecycle), "Screenshots Captured" (paths), "Findings By Severity", "Prioritized Fix List", "Considered And Dismissed" (the verdict=false-positive items, so future runs do not re-flag them), "Remaining Risks / Verify Visually" (include any QA pages that need manual cleanup), and "Approval Status" (pass / pass-with-polish / fail). Do not invent findings beyond those provided. Do not write scratch files into the repo other than the report itself.
